...
class WebHooks
{
    /** Toolbox class providing methods unique to the shop */
    var $_toolbox = null;
    ...
    public function __construct($toolbox)
    {
        ...
        if(!$this->_validateToolbox()){
            throw new Exception("Toolbox could not be loaded.");
        }
    ...
    public function registerAction()
    {
        ...
        $eventList = $this->_toolbox->getEventList();
     ...
     * @return bool
     */
    private function _validateToolbox()
    {
        $toolboxValid = false;
        $toolbox = $this->_toolbox;

        if(isset($toolbox) && $this->_hasRequiredMethods($toolbox)){
            $toolboxValid = true;
        }
    ...
    /**
     * Validates the toolbox object by checking for method names used in this class
     * @param $toolbox
     *
     * @return bool
     */
    private function _hasRequiredMethods($toolbox)
    {
        $requiredMethods = array('getEvent', 'getEventList');
        $methodList = get_class_methods(get_class($toolbox));
